...
(() => {
  ...
  async function toDataUrl(file, targetLongCss) {
    const bitmap = await createImageBitmap(file);
    try {
      // Cap by the slot's LONGEST displayed edge × device pixel ratio, so portrait
      // slots keep enough vertical resolution (the old width-only cap under-resolved
      // tall cards). dpr is clamped to 2–3 to stay retina-sharp without bloating.
      const dpr = Math.min(3, Math.max(2, window.devicePixelRatio || 2));
      const cap = Math.min(MAX_DIM, Math.max(1, Math.round(targetLongCss * dpr)) || MAX_DIM);
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      const gen = ++this._gen;
      try {
        const w = Math.max(this.clientWidth, this.clientHeight) || this.offsetWidth || MAX_DIM;
